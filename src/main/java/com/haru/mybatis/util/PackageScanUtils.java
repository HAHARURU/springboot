...
public class PackageScanUtils {
    public static List<Class> scanClassName(String basePackage) {
        ...
        try {
            urls = Thread.currentThread().getContextClassLoader().getResources(pkgDirName);
            while (urls.hasMoreElements()) {
                ...
                if ("file".equals(protocol)) {
                    ...
                    findAndAddClassesInPackageByFile(basePackage, filePath, clss);
                        ...
                        if (name.startsWith(pkgDirName)) {
                            ...
                            if ((idx != -1)) {
                                if (name.endsWith(".class") && !entry.isDirectory()) {
                                    ...
                                    // 添加到classes
                                    try {
                                        clss.add(Class.forName(pkgName + '.' + className));
     ...
     * @param classes
     */
    static void findAndAddClassesInPackageByFile(String packageName, String packagePath, List<Class> classes) {
        // 获取此包的目录 建立一个File
